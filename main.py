...
@app.route('/criminal_face')
def criminal_face():
    return render_template('criminal_face.html')


@app.route('/criminal_uploader', methods=['GET', 'POST'])
def upload_criminal():
    if request.method == 'POST':
        f = request.files['file']
        f.save(secure_filename('suspect.png'))
        ...
        try:
            ...
            for i in range(0, len(df_db)):
                ...
                img = np.fromstring(img, np.uint8)
                img = cv2.imdecode(img, cv2.IMREAD_COLOR)
                cv2.imwrite('static/captures/user_img.png', img)
                known_image = face_recognition.load_image_file(
                    "static/imgs/user_img.png")
                unknown_image = face_recognition.load_image_file(
                    "suspect.png")
                name_1 = df_db['name'].iloc[i]
